...
@login_required
def direct_assessment_with_error_annotation(request, code=None, campaign_name=None):
    """
    Direct assessment with error annotation view.
    """
    t1 = datetime.now()
    ...
    # If language code has been given, find a free task and assign to user.
    if not current_task:
        current_task = DirectAssessmentWithErrorAnnotationTask.get_task_for_user(
            user=request.user
        )
    ...
    if not current_task:
        ...
        next_task = DirectAssessmentWithErrorAnnotationTask.get_next_free_task_for_language(
            code, campaign, request.user
        )
    ...
    return render(
        request, 'EvalView/direct-assessment-with-error-annotation.html', context
    )
